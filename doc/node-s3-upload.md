...
***AWS S3 이미지 업로드시 이미지 최적화를 진행하고 업로드하는 것이 브라우저에서의 속도가 크게 도움이 됩니다.*** `formidable` 업로드 , `AWS-S3` 업로드는 이 전 작업에서 한번 해보았기 때문에 간단하게 설명하고 이미지 최적화 적업에 대해서 자세히 학습하도록 하겠습니다.
